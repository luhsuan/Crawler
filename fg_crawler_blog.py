...
def singlePage(url):
	...
	#看板名稱ok-------------------------------------------
	Board_name=soup.find('div',{'class','article-meta-left'})
	Board_name=soup.find('a',{'class','tag cat-tag pink'})
	Board_name=Board_name.get_text()

	# print("看板名稱:"+Board_name)

	#日期區間ok-------------------------------------------
	Date_range=soup.find('div',{'class','post-date'})
	Date_range=Date_range.get_text()
	Date_range=Date_range[0:7]#只擷取字串前10字元

	# print("日期區間"+Date_range)

	#文章類型ok-------------------------------------------
	Type=soup.find_all('li',{'class','breadcrumb-item'})
	Type=Type[1].get_text()

	Sub_type=soup.find('div',{'class','article-meta-left'})
	Sub_type=soup.find('a',{'class','tag cat-tag pink'})
	Sub_type=Sub_type.get_text()
	# print("文章類型:"+Type)
	# print("文章子類型:"+Sub_type)
	...
	#文章日期ok----------------------------------------------
	Date=soup.find('div',{'class','post-date'})
	Date=Date.get_text()
	Date=Date[0:10]
	# print("文章日期:"+Date)

	#文章時間ok----------------------------------------------
	Time=soup.find('div',{'class','post-date'})
	Time=Time.get_text()
	Time=Time[11:15]#只擷取字串前10字元
	# print("文章時間:"+Time)

	#[文章作者id]ok----------------------------------------------
	Author=soup.find_all('li',{'class','breadcrumb-item'})
	Author=Author[2].get_text()
...
# ##########################################################
	...
	category=soup.find('div',{'class','content-header'})
	category=category.get_text()
	# print('Category :',category)

	#文章名稱ok-------------------------------------------
	title=soup.find('div',{'class':'fg-card-list'})
	title_url=title.find_all('li',{'class':'fg-card-middle'})
	# title_url=title.find_all('a')
	# print("文章名稱",title_url)

	# 文章發布日期----------------------------------------
	datetime=soup.find_all('div',{'class':'fg-card-date'})
	# print("文章日期",datetime)
	...
	for Date_range in all_article:
		...
		if not os.path.exists(save_path+'/fg/'+category+'/'+Date_range+'.json'):
			all_articles={#創新物件 避免原all_article被覆蓋
				'Source_type':'部落格',
			...
			with open(save_path+'/fg/'+category+'/'+Date_range+'.json','w',encoding='utf-8') as output:
				json.dump([all_articles],output,indent=4,ensure_ascii=False)
...
def main():
	...
	url="https://fgblog.fashionguide.com.tw/category/2/last_posts?page=2"
	getIndex(url)
...
if __name__ == '__main__':
	# main(sys.argv[1])
	main()
#會記錄到多餘html  https://fgblog.fashionguide.com.tw/12488/posts/333417 